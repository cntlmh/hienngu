...
# ---------------- show end  -----------------
# ---------------- export onnx start  -----------------
# You can choose to export only decoder or full pipeline
class FullModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return output[("disp", 0)]  # Return disparity map only
...
# ------------------ show start -----------------
        # ===== Hiển thị và lưu ảnh Depth/Disp =====
        disp_vis = pred_disp / np.max(pred_disp)
        depth_vis = pred_depth_full / np.max(pred_depth_full)
        ...
        plt.tight_layout()
        save_path = f"vis_outputs/depth_disp_{i:04d}.png"
...
# ---------------- export onnx start  -----------------
# ---------- EXPORT TO ONNX ----------
    print("-> Exporting model to ONNX (fixed resolution 640x192)")

    dummy_input = torch.randn(1, 3, 192, 640).cuda()  # (B, C, H, W)


    onnx_path = os.path.join(opt.load_weights_folder, "monodepth_fixed640x192.onnx")

    torch.onnx.export(
        FullModel(encoder, depth_decoder),
        dummy_input,
        onnx_path,
        opset_version=13,
